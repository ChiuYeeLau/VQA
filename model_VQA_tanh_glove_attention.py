...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden, max_words_q, vocabulary_size, drop_out_rate, emb_matrix):
        ...
        self.att_Q_W = tf.Variable(tf.random_uniform([self.input_embedding_size, self.dim_hidden], -0.08,0.08),name='att_Q_W')
        # self.att_Q_b = tf.Variable(tf.random_uniform([self.dim_hidden], -0.08, 0.08), name='att_Q_b')
        ...
        self.att = tf.Variable(tf.random_uniform([self.dim_hidden, 1], -0.08,0.08),name='att')
        ...
        # image-embedding W2
        self.att_image_W = tf.Variable(tf.random_uniform([dim_image, self.dim_hidden], -0.08, 0.08), name='att_image_W')
        self.att_image_b = tf.Variable(tf.random_uniform([dim_hidden], -0.08, 0.08), name='att_image_b')
    ...
    def build_model(self):
        ...
        ques_mask = tf.cast(tf.sign(question), tf.float32)
        # 500 * 26

        # answer vectors <- average of word embeddings
        ans = tf.nn.embedding_lookup(self.embed_ques_W, answer)
        state_ans = tf.reduce_sum(ans, 1)

        # question attention
        ques_emb = tf.nn.embedding_lookup(self.embed_ques_W, question)
        # ques = tf.nn.dropout(ques_emb, 1-self.drop_out_rate)
        # 500 * 26 * 300
        ques_2d = tf.reshape(ques_emb, [-1, self.input_embedding_size])
        # 13000 * 300

        # img = tf.nn.dropout(image, 1-self.drop_out_rate)
        # 500 * 2048

        ques_att_linear_2d = tf.matmul(ques_2d, self.att_Q_W)
        ques_att_linear = tf.reshape(ques_att_linear_2d, [self.batch_size, self.max_words_q, self.dim_hidden])
        # 500 * 26 * 1024
        img_att_linear = tf.nn.xw_plus_b(image, self.att_image_W, self.att_image_b)
        img_att_linear_3d = tf.expand_dims(img_att_linear, 1)
        # 500 * 1 * 1024
        att_linear_3d = ques_att_linear + img_att_linear_3d
        att_3d = tf.tanh(att_linear_3d)
        # 500 * 26 * 1024

        att_2d = tf.reshape(att_3d, [-1, self.dim_hidden])
        # 13000 * 1024
        att_softmax_linear_2d = tf.matmul(att_2d, self.att)
        # 13000 * 1

        att_linear_2d = tf.reshape(att_softmax_linear_2d, [self.batch_size, self.max_words_q])
        att_softmax = tf.nn.softmax(att_linear_2d)
        # 500 * 26
        att_softmax_mask = tf.mul(att_softmax, ques_mask)
        # no normalization
        # 500 * 26
        att_softmax_mask_row_sum = tf.reduce_sum(att_softmax_mask, 1)
        att_softmax_mask_row_sum_T = tf.expand_dims(att_softmax_mask_row_sum, 1)
        # 500 * 1
        att_softmax_2d = att_softmax_mask/att_softmax_mask_row_sum_T
        att_softmax_3d = tf.expand_dims(att_softmax_2d, 1)

        ques_f_3d = tf.batch_matmul(att_softmax_3d, ques_emb)
        state_ques = tf.reshape(ques_f_3d, [self.batch_size, self.input_embedding_size])

        # three inputs: state_ques, state_ans, img
        loss = 0.0
...
# Data input setting
input_img_h5 = './data_img_resnet.h5'
