...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden, max_words_q, vocabulary_size, drop_out_rate, emb_matrix):
        ...
        '''
        self.forward = rnn_cell.LSTMCell(rnn_size, input_embedding_size, use_peepholes=True,state_is_tuple=False)
        self.forward_dropout = rnn_cell.DropoutWrapper(self.forward, output_keep_prob = 1 - self.drop_out_rate)
        self.backward = rnn_cell.LSTMCell(rnn_size, input_embedding_size, use_peepholes=True,state_is_tuple=False)
        self.backward_dropout = rnn_cell.DropoutWrapper(self.backward, output_keep_prob = 1 - self.drop_out_rate)
    ...
    def build_model(self):
        ...
        # state = tf.zeros([self.batch_size, self.rnn_size*self.rnn_layer*2])
        state_que = tf.zeros([self.batch_size/2, self.rnn_size*self.rnn_layer])  #zhe
        state_ans = tf.zeros([self.batch_size/2, self.rnn_size*self.rnn_layer])  #zhe
        loss = 0.0
