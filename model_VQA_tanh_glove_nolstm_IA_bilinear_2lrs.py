...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden, max_words_q, vocabulary_size, drop_out_rate, emb_matrix, QI_dim, QIA_dim):
        ...
        # QI-embedding W3
        self.embed_QI_W = tf.Variable(tf.random_uniform([self.QI_dim, dim_hidden], -0.08, 0.08), name='embed_QI_W')
        self.embed_QI_b = tf.Variable(tf.random_uniform([dim_hidden], -0.08, 0.08), name='embed_QI_b')

        # score-embedding W4
        self.embed_scor_W = tf.Variable(tf.random_uniform([self.QIA_dim, num_output], -0.08, 0.08), name='embed_scor_W')
    ...
    def build_model(self):
        ...
        ## inputs: Q_emb, image_emb, A_emb: 500 * 1024
        Q_emb_3d = tf.expand_dims(Q_emb, 1)
        Q_emb_4d = tf.expand_dims(Q_emb_3d, 2)

        image_emb_3d = tf.expand_dims(image_emb, 1)
        image_emb_4d = tf.expand_dims(image_emb_3d, 2)

        A_emb_3d = tf.expand_dims(A_emb, 1)
        A_emb_4d = tf.expand_dims(A_emb_3d, 2)

        AI = compact_bilinear_pooling_layer(A_emb_4d, image_emb_4d, self.QI_dim)

        # QI = tf.mul(Q_emb, image_emb)

        AI_drop = tf.nn.dropout(AI, 1-self.drop_out_rate)
        AI_linear = tf.nn.xw_plus_b(AI_drop, self.embed_QI_W, self.embed_QI_b)
        AI_emb = tf.tanh(AI_linear)

        AI_emb_3d = tf.expand_dims(AI_emb, 1)
        AI_emb_4d = tf.expand_dims(AI_emb_3d, 2)

        QIA = compact_bilinear_pooling_layer(AI_emb_4d, Q_emb_4d, self.QIA_dim)
