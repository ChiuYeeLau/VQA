...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden_QI, dim_hidden_QIA, max_words_q, vocabulary_size, drop_out_rate, emb_matrix, decay, initial_bound):
...
batch_size = 1728            # batch_size for each iterations
...
def train():
    ...
    train_target_pos, train_target_neg = np.where(train_data['target'][:,0] == 1)[0], np.where(train_data['target'][:,0] == 0)[0]
    ...
    for itr in range(max_itr):
        ...
        pos_index = np.random.choice(train_target_pos, batch_size/3, replace = False)
        np.random.shuffle(pos_index)
        neg_index = np.random.choice(train_target_neg, batch_size*2/3, replace = False)
