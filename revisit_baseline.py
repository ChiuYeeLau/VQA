...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden_QI, dim_hidden_QIA, max_words_q, vocabulary_size, drop_out_rate, emb_matrix, decay):
        ...
        self.layer1_W = tf.Variable(tf.random_uniform([self.input_embedding_size *2 + self.dim_image, self.dim_hidden_QI], -0.08,0.08,seed = random_seed),name='layer1_W')
        self.layer1_b = tf.Variable(tf.random_uniform([self.dim_hidden_QI], -0.08, 0.08,seed = random_seed), name='layer1_b')

        self.layer2_W = tf.Variable(tf.random_uniform([self.dim_hidden_QI, 2], -0.08,0.08,seed = random_seed),name='layer2_W')
        self.layer2_b = tf.Variable(tf.random_uniform([2], -0.08, 0.08,seed = random_seed), name='layer2_b')
    ...
    def build_model(self, is_training):
        ...
        ques_inputs = tf.nn.embedding_lookup(self.embed_ques_W, question)
        ans_inputs = tf.nn.embedding_lookup(self.embed_ques_W, answer)
        state_que = tf.reduce_sum(ques_inputs, 1)
...
# Data input setting
input_img_h5 = './data_img_resnet_wikiGlove_shuffle.h5'
input_ques_h5 = './data_prepro_wikiGlove_shuffle.h5'
...
def train():
    ...
    tf_loss, tf_image, tf_question, tf_answer, tf_label = model.build_model(True)
    ...
    for itr in range(max_itr):
        ...
        if np.mod(itr, 600) == 0:
            ...
            tf_proba_test, tf_image_test, tf_question_test, tf_answer_test = model.build_generator(False)
